...
export class AgentsController {
  ...
  @Post('action/create')
  @ApiOperation({ summary: '创建Agent', description: '创建一个新的Agent并关联角色' })
  ...
  @Post('action/list')
  @ApiOperation({ summary: '查询Agent列表', description: '分页查询Agent列表，支持搜索' })
  ...
  @Post('action/get')
  @ApiOperation({ summary: '获取Agent详情', description: '根据ID获取Agent详细信息及关联的角色和能力' })
  @ApiResponse({ status: 200, description: '查询成功' })
  ...
  @Post('action/update')
  @ApiOperation({ summary: '更新Agent', description: '更新Agent信息和关联的角色' })
  ...
  @Post('action/execute')
  @ApiOperation({ summary: '执行Agent', description: '手动触发Agent执行，Agent会根据AI决策执行相应能力' })
