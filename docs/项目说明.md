...
Agent 是为了解决具体问题而创建的综合解决方案：
- 可以包含多个角色（如 DBA、项目经理等）
- 拥有调用所有关联角色能力的权限
- 根据 AI 决策自动选择执行哪些能力
- 响应时间线事件自动执行任务
...
- `POST /agents/action/list` - 查询 Agent 列表
- `POST /agents/action/get` - 获取 Agent 详情
...
时间线通知器作为定时生命周期事件通知 Agent：
- Agent 自己调用 AI 决策来决定执行哪些能力
- 支持多种事件类型：定时、间隔、Cron 表达式
- 自动调度和执行
- 完整的状态追踪
...
- `POST /timeline/action/create` - 创建时间线事件
- `POST /timeline/action/list` - 查询事件列表
- `POST /timeline/action/get` - 获取事件详情
- `POST /timeline/action/update` - 更新事件
- `POST /timeline/action/delete` - 删除事件
- `POST /timeline/action/execute` - 手动执行事件
...
# 数据库备份能力
curl -X POST http://localhost:3000/api/v1/capabilities/action/create \
  -d '{"name":"数据库备份","description":"执行数据库备份","prompt":"..."}'

# 性能监控能力
curl -X POST http://localhost:3000/api/v1/capabilities/action/create \
  -d '{"name":"性能监控","description":"监控数据库性能","prompt":"..."}'
```
...
- [Agent 和时间线完整文档](./AGENT_TIMELINE.md)
- [API 文档](./API.md)
- [Swagger 使用指南](./SWAGGER.md)
